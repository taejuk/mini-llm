...
std::vector<T> read_all_bin(const std::string& path) {
    ...
    if (!ifs) {
        ...
        std::cerr << "Generate batch prefill golden files first:\n";
        std::cerr << "  python scripts/dump_gpt2_batch_prefill_golden.py\n";
        std::exit(1);
    }
...
void prepare_batched_prefill_kv_blocks(
...
std::vector<std::unique_ptr<Rt::Request>> make_requests_from_flat_tokens(
    ...
    for (size_t i = 0; i < prompt_lens.size(); ++i) {
        ...
        if (len <= 0) {
            std::cerr << "invalid prompt length at row " << i
                      << ": " << len << "\n";
            std::exit(1);
        }

        if (len >= C::MAX_SEQ) {
            std::cerr << "prompt length exceeds MAX_SEQ-safe range at row "
                      << i << ": " << len << "\n";
            std::exit(1);
        }
...
int main() {
    auto flat_input_ids =
        read_all_bin<int32_t>(
            model_case_path("batch_prefill_input_ids.bin")
        );

    auto prompt_lens =
        read_all_bin<int32_t>(
            model_case_path("batch_prefill_prompt_lens.bin")
        );

    auto expected_logits =
        read_all_bin<float>(
            model_case_path("batch_prefill_expected_logits.bin")
        );

    if (prompt_lens.empty()) {
        std::cerr << "batch_prefill_prompt_lens.bin is empty\n";
        return 1;
    }
    ...
    if (expected_logits.size() != expected_logit_count) {
        std::cerr
            << "batch_prefill_expected_logits.bin size mismatch: got="
    ...
    prepare_batched_prefill_kv_blocks(reqs);

    auto& model = Model::GPT2Model::get();

    std::vector<float> got_logits =
        model.prefill_logits_for_test(reqs);
    ...
    std::cout << "batch_prefill avg_mean_abs_error = "
              << avg_mean_abs << "\n";
    std::cout << "batch_prefill global_max_abs     = "
              << global_max_abs << "\n";
    std::cout << "batch_prefill min_cosine         = "
              << min_cosine << "\n";

    if (!all_pass) {
        std::cerr << "[FAIL] gpt2_batch_prefill_logits\n";
        return 1;
    }

    std::cout << "[PASS] gpt2_batch_prefill_logits\n";
    return 0;
}