...
std::vector<std::unique_ptr<Rt::Request>> make_prefill_requests(
    int batch_size,
    int prompt_len,
    ...
    for (int i = 0; i < batch_size; ++i) {
        ...
        if (!ok) {
            std::cerr << "allocate_prefill failed for prefill request\n";
            std::exit(1);
        }

        req->state = Rt::RequestState::PrefillRunning;
...
std::vector<std::unique_ptr<Rt::Request>> make_decode_requests(
    ...
    // If cached_tokens is on a KV block boundary, one extra block per layer is needed.
    for (auto& req : reqs) {
...
void run_overlap_suite(
    ...
    std::vector<int> used_blocks = collect_used_blocks(reqs);
    ...
    for (int blocks : BLOCK_COUNTS) {
        ...
        auto run_overlap = [&]() -> double {
            ...
            // Current GPT2Model workload runs as-is on the default stream.
            // swap_in runs on a non-blocking stream.
            run_workload();
        ...
        double max_possible_saved =
            std::min(swap_res.avg_ms, workload_res.avg_ms);
...
int main(int argc, char** argv) {
    ...
    if (argc >= 4) batch_size = std::atoi(argv[3]);
    if (argc >= 5) prompt_len = std::atoi(argv[4]);
    if (argc >= 6) seed = static_cast<unsigned int>(std::atoi(argv[5]));

    if (iterations <= 0 || warmup < 0) {
        std::cerr
            << "usage: ./bench_real_prefill_decode_swap_overlap "
            << "[iterations] [warmup] [batch_size] [prompt_len] [seed]\n";
        return 1;
    }
    ...
    std::cout << "NOTE: prefill and decode use current GPT2Model implementation as-is.\n";
    std::cout << "NOTE: swap_in uses H2D to GPU staging + vec4 scatter on a non-blocking stream.\n\n";
...

    {
        ...
        auto run_prefill = [&]() {
            (void)model.prefill(prefill_reqs);
        };
        ...
        free_requests(prefill_reqs, allocator);
    }

    {
        ...
        free_requests(decode_reqs, allocator);
    }
